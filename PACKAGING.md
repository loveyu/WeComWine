...
`5.0.7.6005deepin11` 的适配目录、`files.7z`、企业微信 5.0.7.6005 预制前缀、
原生 RichEdit、注册表和辅助文件。该单包不下载、封装或安装腾讯官方
5.0.10.6025。企业微信运行路径使用完整 Deepin Wine 10.14 和匹配的
`WINEPREDLL` 适配层；Wine 11 文件仍保留在单包中供 Portal 辅助路径使用，但不与
Wine 10 的窗口栈混合加载。

Flatpak 同时封装文泉驿微米黑字体、轮廓版 Noto Emoji、`deepin-wine-helper`
资源、7z 与所需原生库 ABI；启动时把 Noto Emoji 注册到独立 Wine 前缀，追加为
Tahoma 的字体回退，并将 `Segoe UI Emoji` 替换为该字体，避免 emoji 缺字显示为空白
或方块。所有 WineDbg 入口在构建和
前缀迁移时移除，禁止以调试模式运行。HTTP/HTTPS
...
文件夹使用标准 FileManager1 接口，不绑定特定文件管理器。
首次启动会迁移既有 Deepin 前缀，保留登录数据并将旧 Wine 系统目录留作可恢复
备份。完整 Deepin 企业微信代码只进入本机私有包，不进入 Git。
包内企业微信 5.0.7 使用窄匹配 CEF 兼容补丁；启动环境固定
...
独立 Portal runner 所需的腾讯企业微信安装包由目标机器根据固定 HTTPS URL 下载
并校验 SHA-256，不能进入 Git、源码归档、公开 Flatpak 仓库或公开发行附件。
Deepin 单包不叠加该安装包，只使用 Deepin 包内的匹配客户端；上述本机私有包仅限
用户自有机器使用，不得上传或再分发。
...
- `make deepin-flatpak-local` 从 Deepin/统信官方应用商店下载并校验 Deepin 来源
  `5.0.7.6005deepin11` 包，携带固定摘要的企业微信 5.0.7.6005 预制前缀、完整
  Deepin Wine 10.14 引擎和匹配适配层，生成并安装独立应用 ID 的本地测试
  Flatpak。构建过程不再获取腾讯 5.0.10 安装包。生成物位于
...
- `scripts/switch-to-deepin-runner.sh` 停止当前 runner，直接从 Flatpak 内置的
  ...
  `--start` 才会以正常模式启动。该入口不会运行 5.0.10 安装器；Deepin 模式启用
  适用于包内 5.0.7 的 CEF 补丁、
