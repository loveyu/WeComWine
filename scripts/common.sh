...
PORTAL_WINE_VERSION="11.16"
PORTAL_WINE_SHA256="c66e2090343dcd727f7f7fd2f87ee0bfb0b118790c1d745ab7b8a4c3a4197f2f"
PORTAL_WINE_URL="https://dl.winehq.org/wine/source/11.x/wine-11.16.tar.xz"
PORTAL_PATCHSET="mr10060-f36314a-wecom11"
DEEPIN_FLATPAK_APP="io.github.loveyu.WeComWine.Deepin"
DEEPIN_FLATPAK_BRANCH="stable-25.08"
DEEPIN_WINEPREFIX_SANDBOX="/var/data/wine-wecom-deepin"
DEEPIN_WINEPREFIX_HOST="${HOME}/.var/app/${DEEPIN_FLATPAK_APP}/data/wine-wecom-deepin"
...
if [[ -s "${RUNNER_STATE_FILE}" ]]; then
    ...
    elif [[ "${configured_runner}" == "${DEEPIN_FLATPAK_APP}" ]]; then
        ACTIVE_FLATPAK_APP="${DEEPIN_FLATPAK_APP}"
        ACTIVE_FLATPAK_BRANCH="${DEEPIN_FLATPAK_BRANCH}"
    fi
fi

if [[ "${ACTIVE_FLATPAK_APP}" == "${DEEPIN_FLATPAK_APP}" ]]; then
    WINEPREFIX_SANDBOX="${DEEPIN_WINEPREFIX_SANDBOX}"
    WINEPREFIX_HOST="${DEEPIN_WINEPREFIX_HOST}"
elif [[ -f "${SHARED_WINEPREFIX_HOST}/system.reg" ]]; then
...
flatpak_wine_scaled() {
    ...
    if [[ "${ACTIVE_FLATPAK_APP}" == "${DEEPIN_FLATPAK_APP}" ]]; then
        # Keep registry setup and the application on the same Deepin Wine 10
        # wineserver.  Mixing a Wine 11 reg.exe process into this prefix can
        # load a different user-driver ABI before WeCom starts.
        flatpak_wine sh -c '
            /app/share/wecom-deepin/migrate-prefix-to-wine10.sh
            /app/bin/deepin-wine reg.exe add "HKCU\Control Panel\Desktop" \
