...
- 安装或原地升级时自动备份并清理旧版本遗留的
  `wecom-wine-system-frame` KWin 规则，不修改用户的其他窗口规则。
- 最大化时仅在企业微信主窗口位于前台才映射其自绘顶栏。该顶栏是绕过 KWin
  管理的 X11 窗口；前台切换时同步隐藏/恢复，避免它覆盖其他应用的窗口按钮并
  截获点击。
- 将企业微信主窗口仅有的 48×48 低清图标替换为随包提供的 16～256 像素多尺寸
  图标，并在消息提醒期间持续校正应用重新提交的异常图标，避免任务栏图标锯齿
  或闪成黑色方块。
...
桌面文件关联；设置 `WECOM_MANAGE_WINDOW_ICON=0` 可临时关闭。该功能依赖宿主
已有的 `python3`、ImageMagick `magick` 和 X11 `libX11`，缺失时自动降级，不
阻止企业微信启动。
